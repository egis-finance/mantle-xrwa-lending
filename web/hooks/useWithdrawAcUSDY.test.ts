...
import { useWithdrawAcUSDY } from './useWithdrawAcUSDY';
...
const mockWithdrawHash = '0xWithdrawHash' as `0x${string}`;

describe('useWithdrawAcUSDY', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    mockWriteContract.mockResolvedValue(mockWithdrawHash);
  ...
  describe('Guardrails', () => {
    it('throws when wallet not connected', async () => {
      ...
      const { result } = renderHook(() => useWithdrawAcUSDY(mockMarketParams));

      await act(async () => {
        await expect(result.current.withdrawCollateral(1000000000000000000n)).rejects.toThrow('No Ethereum wallet connected');
      });
    ...
    it('throws when Morpho contract unconfigured', async () => {
      ...
      await act(async () => {
        await expect(result.current.withdrawCollateral(1000000000000000000n)).rejects.toThrow('Morpho contract not configured');
      });
    });

    it('throws when marketParams not loaded', async () => {
      const { result } = renderHook(() => useWithdrawAcUSDY(undefined));

      await act(async () => {
        await expect(result.current.withdrawCollateral(1000000000000000000n)).rejects.toThrow('Market params not loaded');
      });
      expect(result.current.status).toBe('error');
    });
    ...
    it('throws when amount is zero', async () => {
      ...
      await act(async () => {
        await expect(result.current.withdrawCollateral(0n)).rejects.toThrow('Amount must be greater than zero');
      });
      expect(result.current.status).toBe('error');
    });
    ...
    it('throws when amount is negative', async () => {
      ...
      await act(async () => {
        await expect(result.current.withdrawCollateral(-1n)).rejects.toThrow('Amount must be greater than zero');
      });
      expect(result.current.status).toBe('error');
    });
  });
  ...
  describe('Status Transitions', () => {
    it('progresses through status states on success', async () => {
      ...
      await act(async () => {
        await result.current.withdrawCollateral(1000000000000000000n);
      });

      expect(result.current.status).toBe('success');
      expect(result.current.txHash).toBe(mockWithdrawHash);
    });

    it('sets status to error on failure', async () => {
      mockWriteContract.mockRejectedValueOnce(new Error('Insufficient health factor'));

      const { result } = renderHook(() => useWithdrawAcUSDY(mockMarketParams));

      await act(async () => {
        await expect(result.current.withdrawCollateral(1000000000000000000n)).rejects.toThrow('Insufficient health factor');
      });
      expect(result.current.status).toBe('error');
      expect(result.current.error?.message).toBe('Insufficient health factor');
    });
  });

  describe('Contract Calls', () => {
    it('calls withdrawCollateral with correct args (no approval needed)', async () => {
      const { result } = renderHook(() => useWithdrawAcUSDY(mockMarketParams));

      await act(async () => {
        await result.current.withdrawCollateral(500000000000000000000n); // 500 AcUSDY
      });

      // Verify only one contract call (withdrawCollateral, no approve)
      ...
      expect(mockWriteContract).toHaveBeenCalledWith({
        ...
        args: [
          mockMarketParams,
          500000000000000000000n, // assets (AcUSDY amount)
  ...
  describe('Reset', () => {
    it('resets status and error on reset', async () => {
      ...
      await act(async () => {
        await expect(result.current.withdrawCollateral(1000000000000000000n)).rejects.toThrow();
      });
      expect(result.current.status).toBe('error');
  ...
  describe('Status Messages', () => {
    ...
    it('returns correct status message for success', async () => {
      ...
      expect(result.current.statusMessage).toBe('Collateral withdrawn!');
    });
  });
});
