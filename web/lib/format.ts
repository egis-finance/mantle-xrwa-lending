...
// Formats a number as a dollar value with commas (no decimals for integers >= 1)
export function formatDollarValue(value: number | null): string {
  if (value === null || value === undefined) return '$0'
  if (value === 0) return '$0'
  // Use integer formatting for whole numbers, otherwise show 2 decimals for small values
  const formatted = Number.isInteger(value) || value >= 1
