...
const usdyAddress: Address =
  (process.env.NEXT_PUBLIC_MANTLE_USDY as Address | undefined) ?? DEFAULT_USDY_ADDRESS;
...
export function HardcodedUsdyBalance({ address, label }: HardcodedUsdyBalanceProps) {
  ...
  const { data, isLoading, isError } = useMultiChainBatchRead<[bigint, number]>({
  ...
  const balanceInfo = useMemo(() => {
    ...
    if (isLoading) {
      return { value: 'Loading...', shortAddress };
    }
    ...
    return { value: display, shortAddress };
      ...
      <div className="flex items-center gap-4 px-4 py-3">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-brand-muted/80">
              {label}
            </span>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="flex items-center gap-2 rounded-md bg-brand-light/30 px-2 py-1 ring-1 ring-brand-light/50">
            <span className="font-mono text-xs font-medium text-brand-dark/80">
              {balanceInfo.shortAddress}
            </span>
